      ...
      <section id="projects" className="py-32 relative">
        ...
        <div className="container relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-16">
            <ProjectCard
              title="Omega Nova — AI-Powered Geopolitical & Risk Intelligence Platform"
              description="Enterprise SaaS for real-time risk forecasting, simulation, and decision support. A multi-tenant AI platform that helps enterprise analysts forecast geopolitical, commodity, and cyber risks through LLM-driven simulations, real-time signal enrichment, and calibrated probability modeling."
              tags={[
                "Next.js",
                "TypeScript",
                "NestJS",
              ...
              isConfidential
              confidentialNote="Built end-to-end as the sole developer for a private client under NDA. Source, screenshots, and the live deployment can't be shared publicly."
              details={{
                role: "Sole Full Stack Engineer",
                duration: "6-week intensive sprint cycle",
                status: "In production — 10–20 concurrent enterprise users",
                overview: [
                  "Omega Nova is an enterprise-grade risk intelligence platform that combines large language models with real-time market, geopolitical, and cyber-threat signals to produce calibrated forecasts and scenario simulations. Analysts can ask natural-language questions, run multi-agent simulations, receive automated morning briefs, and track evolving risk indicators across domains (commodities, energy, VIX, polling, OGWI indices, cyber threats).",
                  "I designed and built the entire system — frontend, backend, infrastructure, LLM prompt engineering, and data pipelines — iterating directly with the client over a 6-week intensive sprint cycle to ship 50+ production features.",
                ],
                highlights: [
                  "Sole developer on a production system serving 10–20 concurrent enterprise users.",
                  "Built a structured LLM simulation engine with 6 mandatory forcing functions (historical analog, agent deviation, leading indicators, 2nd/3rd-order effects, commitment-action gap, blind-spot detection) to reduce hallucination and improve calibration.",
                  "Implemented dynamic context enrichment: an intent classifier routes each query to fetch the right real-time signals (commodities, energy, VIX, cyber, polls) before the LLM call.",
                  "Designed a 3-zone workspace UI for simulation analysis — approved by client on first review.",
                  "Shipped multi-tenant RBAC with role-scoped permissions across simulations, agents, notifications, and knowledge base.",
                  "Built automated morning briefs with 5×/day scheduled cadence and per-user calibration.",
                  "Scaled infrastructure for the user base: AWS Fargate (1 vCPU / 2 GB) with 1→3 autoscaling and RDS t3.small.",
                  "Maintained a type-safe codebase end-to-end with TypeScript and Prisma.",
                ],
                architecture: [
                  {
                    title: "Frontend",
                    body: "Next.js (App Router) + TypeScript + TailwindCSS. Server components for data-heavy dashboards, client components for interactive simulation workspaces. Optimistic UI updates for chat and simulation flows.",
                  },
                  {
                    title: "Backend",
                    body: "NestJS, API routes + Prisma ORM over PostgreSQL. Domain-driven structure: separate modules for simulations, agents, knowledge base, notifications, and orchestration.",
                  },
                  {
                    title: "LLM Layer",
                    body: "Anthropic Claude as the primary model with structured prompt templates. Each mode (Ask, Standard, Simulation) has its own prompt contract with internal forcing functions. An intent classifier sits in front of the LLM to enrich context with relevant real-time signals.",
                  },
                  {
                    title: "Data Pipeline",
                    body: "Scheduled jobs pull external indices (commodities, VIX, OGWI, polling) into Postgres. The enrichment layer joins these with user queries on-the-fly.",
                  },
                  {
                    title: "Infrastructure",
                    body: "AWS Fargate for the app tier with horizontal autoscaling, RDS Postgres for persistence, CloudWatch for observability. Zero-downtime deploys via task revision rollover.",
                  },
                  {
                    title: "Auth & RBAC",
                    body: "Centralized can() permission resolver with per-resource, per-action checks. ALL_ACCESS, ADMIN, and scoped roles share the same gate, preventing privilege drift.",
                  },
                ],
                techStack: [
                  {
                  ...
                  {
                    category: "Backend",
                    items: ["NestJS", "Next.js API routes", "Prisma ORM"],
                  },
                  {
                    category: "Database",
                    items: ["PostgreSQL (AWS RDS)"],
                  },
                  {
                    category: "AI / LLM",
                    items: [
                  ...
                  },
                  {
                    ...
                    items: ["AWS Fargate", "RDS", "CloudWatch", "ECR"],
                  },
                  {
                    category: "Auth",
                    items: ["RBAC", "Session management"],
                  },
                  {
                ...
                challenges: [
                  {
                    problem:
                      "LLM hallucination on quantitative data — the model occasionally fabricated specific numbers (e.g., oil prices) when asked open-ended questions.",
                    solution:
                      "Built an intent classifier that detects quantitative queries and forces real-time data fetching before the LLM call. The LLM receives the actual number as grounded context rather than guessing.",
                  },
                  {
                    problem:
                      "Forecast calibration — early simulations were qualitatively rich but poorly calibrated: too confident, too vague on probabilities.",
                    solution:
                      "Designed 6 mandatory forcing functions embedded in the simulation prompt — historical analog matching, agent deviation tracking, leading indicators, 2nd/3rd-order effects, commitment-action gap analysis, and blind-spot detection. Output became measurably more grounded and actionable.",
                  },
                  {
                    problem:
                      "Permission bug invisible to admins — a missing :view case in the permission resolver broke navigation for ALL_ACCESS users while remaining invisible to ADMIN testers.",
                    solution:
                      "Refactored can() to enumerate every resource × action pair explicitly, added a permission-matrix test that runs every role against every resource, and documented the contract.",
                  },
                  {
                    problem:
                      "Rapid client iteration without breaking production — daily feature batches with same-day deploys created high regression risk.",
                    solution:
                      "Adopted small-commit discipline (4-commit grouping per feature batch), kept TypeScript strict, and ran typecheck + smoke tests before every deploy. Shipped 50+ features over 6 weeks with zero rollbacks.",
                  },
                  {
                    problem:
                      "Scaling for concurrent demo users — initial single-task Fargate config couldn't handle 10–20 simultaneous analysts hitting LLM endpoints.",
                    solution:
                      "Bumped task size to 1 vCPU / 2 GB, configured autoscaling (1→3 tasks) on CPU + request-count metrics, and upgraded RDS to t3.small. Sustained load test with no latency spikes.",
                  },
                  {
                    problem:
                      "Memory across conversation threads — context wasn't persisting correctly across sessions, breaking long-running analyst workflows.",
                    solution:
                      "Implemented a per-thread memory store with hierarchical context windows — recent messages verbatim, older messages summarized — keeping prompt sizes bounded while preserving continuity.",
                  },
                ],
              }}
            />
